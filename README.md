...
Для проверки удаления старых бэкапов можно воспользоваться командой

```bash
touch -d "10 days ago" backups/config-2026-09-01-12-00-00.tar.gz
...
Настройки Uptime Kuma сохраняются в Docker volume, поэтому после перезапуска контейнера они не пропадут.

Автоматическое создание монитора не добавлялось, чтобы не усложнять проект и не хранить готовую базу данных с настройками в репозитории.
